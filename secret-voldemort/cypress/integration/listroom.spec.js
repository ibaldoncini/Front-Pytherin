  ...
  describe("User is logged in", () =>{
    it("want to see available rooms", ()=>{
      ...
      it("Lookin for a room that is full", () => {
        ...
        cy.get('input[type=submit]').click()
        cy.get('button[id=join]').click()
        cy.get('button[id=JoinTESTFULLjoin]').click()
        cy.visit(rootpath)
        cy.get('button[id=logout]').click()
        // 2
        cy.get('input[type=email]')
          .type('player3@email.com')
        cy.get('input[type=password')
          .type('Heladera65')
        cy.get('input[type=submit]').click()
        cy.get('button[id=join]').click()
        ...
        cy.get('input[type=submit]').click()
        cy.get('button[id=join]').click()
        cy.get('div[id=JoinTESTFULLjoin]').should('not.contain','JoinTESTFULLjoin')
